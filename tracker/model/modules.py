...
class HiddenUpdater(nn.Module):
    ...
    def forward(self, g, h):
        ...
        update_gate = torch.sigmoid(
            values[:, :, self.hidden_dim : self.hidden_dim * 2]
        )
...
class ValueEncoder(nn.Module):
    def __init__(self, value_dim, hidden_dim, single_object=False):
        ...
        network = resnet.resnet18(
            pretrained=True, extra_dim=1 if single_object else 2
        )
    ...
    def forward(
        self, image, image_feat_f16, h, masks, others, is_deep_update=True
    ):
...
class UpsampleBlock(nn.Module):
    def __init__(self, skip_dim, g_up_dim, g_out_dim, scale_factor=2):
        ...
        self.skip_conv = nn.Conv2d(
            skip_dim, g_up_dim, kernel_size=3, padding=1
        )
        ...
        if h_out and self.hidden_update is not None:
            g4 = torch.cat(
                [
                    g4,
                    logits.view(
                        batch_size, num_objects, 1, *logits.shape[-2:]
                    ),
                ],
                2,
            )
