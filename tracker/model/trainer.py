...
class XMemTrainer:
    def __init__(
        self, config, logger=None, save_path=None, local_rank=0, world_size=1
    ):
        self.config = config
        ...
        if logger is not None:
            ...
            self.logger.log_string(
                'model_size',
                str(
                    sum([param.nelement() for param in self.XMem.parameters()])
                ),
            )
    ...
    def do_pass(self, data, max_it, it=0):
        ...
        with torch.cuda.amp.autocast(enabled=self.config['amp']):
            ...
            key, shrinkage, selection, f16, f8, f4 = self.XMem(
                'encode_key', frames
            )
                ...
                if (it) % self.log_text_interval == 0 and it != 0:
                    ...
                    if self.logger is not None:
                        ...
                        self.logger.log_metrics(
                            'train',
                            'time',
                            (time_spent) / self.log_text_interval,
                            it,
                        )
    ...
    def load_checkpoint(self, path):
        ...
        print(
            'Network weights, optimizer states, and scheduler states loaded.'
        )
