...
class MemoryManager:
    ...
    def __init__(self, config):
        ...
        if self.enable_long_term:
            self.long_mem = KeyValueMemoryStore(
                count_usage=self.enable_long_term_usage
            )
    ...
    def match_memory(self, query_key, selection):
        ...
        selection = (
            selection.flatten(start_dim=2) if selection is not None else None
        )
                ...
                if gi < self.long_mem.num_groups:
                    ...
                    affinity_one_group = do_softmax(
                        torch.cat(
                            [
                                long_mem_similarity[
                                    :, -self.long_mem.get_v_size(gi) :
                                ],
                                work_mem_similarity[
                                    :, -self.work_mem.get_v_size(gi) :
                                ],
                            ],
                            1,
                        ),
            ...
            for gi, gv in enumerate(self.work_mem.value):
                ...
                if gi < self.long_mem.num_groups:
                    all_memory_value.append(
                        torch.cat(
                            [self.long_mem.value[gi], self.work_mem.value[gi]],
                            -1,
                        )
                    )
                else:
    ...
    def add_memory(self, key, shrinkage, value, objects, selection=None):
        ...
        if selection is not None:
            if not self.enable_long_term:
                warnings.warn(
                    'the selection factor is only needed in long-term mode',
                    UserWarning,
                )
    ...
    def compress_features(self):
        ...
        for gv in self.work_mem.value:
            ...
            if mem_size_in_this_group == total_work_mem_size:
                ...
                candidate_value.append(
                    gv[:, :, HW : -self.min_work_elements + HW]
                )
            else:
        ...
        self.work_mem.sieve_by_range(
            HW,
            -self.min_work_elements + HW,
            min_size=self.min_work_elements + HW,
        )
    ...
    def consolidation(
        ...
        affinity = [
            aff if aff is None or aff.shape[-1] > 0 else None
            for aff in affinity
        ]
